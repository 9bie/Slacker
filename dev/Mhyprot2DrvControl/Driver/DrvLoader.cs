...
namespace MhyProt2Drv.Driver
{
    public class DrvLoader
    {
        ...
        public bool Load()
        {
            if (!System.IO.File.Exists(DriverFileName) && System.IO.File.Exists(System.IO.Path.Combine(Environment.CurrentDirectory, "mhyprot2.sys")))
            {
                CopyFiles();
            }
            else if(!System.IO.File.Exists(DriverFileName) && !System.IO.File.Exists(System.IO.Path.Combine(Environment.CurrentDirectory, "mhyprot2.sys")))
            {
                Console.WriteLine("[!] 无法找到驱动文件");
